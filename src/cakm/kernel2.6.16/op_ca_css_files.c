...
ssize_t ca_css_ppid_read(struct file * file, char __user * buf, size_t count, loff_t * offset)
{
	return oprofilefs_ulong_to_user(ca_css_ppid, buf, count, offset);
}

ssize_t ca_css_ppid_write(struct file * file, char const __user * buf, size_t count, loff_t * offset)
{
	unsigned long val;
	int retval;
...
struct file_operations ca_css_ppid_fops = {
	.read		= ca_css_ppid_read,
	.write		= ca_css_ppid_write
};
