...
static void get_anon_maps(struct transient * trans)
{
	...
	if (!fp) {
		/* CA Modification:
		 * If no mapping information is available,
		 * we still want to keep these samples.
		 */
		char name[MAX_IMAGE_NAME_SIZE + 1];
		strcpy(name, "anon (no mapping)");
		add_anon_mapping(trans, 0, 0, name);
		return;
	}
...
anon_match(struct transient const * trans, struct anon_mapping const * anon)
{
	...
	/* CA Modification:
	 * Check if it is the case when no 
	 * memory mapping information available.
	 */
	if (anon->start == 0 && anon->end == 0)
		return 1;
