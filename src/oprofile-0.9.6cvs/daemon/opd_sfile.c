...
extern int opd_mux_norm;

extern struct calog * cacss_log;
...
#ifdef WITH_CA_AGENT
	if (trans->ca_jit) {
		val ^= trans->ca_jit->start >> VMA_SHIFT;
		val ^= trans->ca_jit->end >> (VMA_SHIFT + 1);
	}
#endif
...
#ifdef WITH_CA_AGENT
	 ,struct ca_jit_mapping const * ca_jit
#endif
)
{
...
struct sfile * sfile_find(struct transient const * trans)
{
...
#ifdef ENABLE_CA_CSS
	if (trans->tracing != CALLGRAPH_EVENT) {
#else
	...
	if (trans->in_kernel) {
		...
		if (vsamples) {
			...
			printf("No anon/ca_jit map for pc %llx, app %s.\n",
			       trans->pc, app);
		}
...
static odb_t * get_file(struct transient const * trans, int is_cg)
{
	...
	unsigned long hash;
	odb_t * file;
	unsigned long counter = trans->event;

#ifdef ENABLE_CA_CSS
	if (counter == CALLGRAPH_EVENT) {
		//TODO: Fix me. We need to find a way to log the events properly.
		//counter = 1;	
		return NULL;
	}
#endif
...
static void verbose_print_sample(struct sfile * sf, vma_t pc, uint counter)
{
	...
	if (sf->anon) {
		...
		printf("ca_jit (tgid %u, %s, ", sf->ca_jit->tgid, verbose_cookie(sf->cookie));
#endif
	} else {
...
static void verbose_sample(struct transient const * trans, vma_t pc)
{
	printf("Sample ");
#ifdef WITH_CA_AGENT
	if (trans->current->ca_jit)
		printf("(ca_jit trans->pc:%llx) ", trans->pc);
#endif
...
#ifdef ENABLE_CA_CSS
/**
 * Logs CA Call-Stack Sampling (CSS) record
 */
static void sfile_log_callgraph(struct transient const * trans)
{
	calog_data entry;
	cacss_data data;

	if (trans->ca_tracing_depth) {
		/* This can happen if kernel sample falls through the cracks,
		 * see opd_put_sample() */
		if (trans->last) 
			sfile_log_arc(trans);

		if (trans->ca_tracing_depth == 1) {
			//add_cg_data(trans->tgid, trans->ca_last_cookie, verbose_cookie(trans->ca_last_cookie), trans->last_pc, 1); 

			/* Setup entry */
			entry.cookie     = trans->ca_last_cookie;
			entry.app_cookie = 0; // Not used
			entry.cpu        = trans->cpu;
			entry.tgid       = trans->tgid;
			entry.tid        = trans->tid;
			entry.offset     = trans->last_pc;
			entry.cnt        = 1;

			/* Setup data */
			data.cg_type = 1;

			calog_add_data(cacss_log, 
					&entry, 
					&data, 
					sizeof(cacss_data), 
					verbose_cookie(trans->ca_last_cookie), 
					NULL);
		}				

		//add_cg_data(trans->tgid, trans->cookie, 
		//	verbose_cookie(trans->cookie), trans->pc, 0); 
...
{
	sfile_log_sample_count(trans, 1 * opd_mux_norm);
}
...
#ifdef ENABLE_CA_CSS
	if (trans->event == CALLGRAPH_EVENT) {
		sfile_log_callgraph(trans);
		return;
	}
#else
	if (trans->tracing == TRACING_ON) {
...
#ifdef WITH_CA_AGENT
static int is_sfile_ca_jit(struct sfile *sf, void * data __attribute__((unused)))
{
	return !!sf->ca_jit;
}
#endif
...
#ifdef WITH_CA_AGENT
void sfile_clear_ca_jit(void)
{
	for_each_sfile(is_sfile_ca_jit, NULL);
}
#endif
