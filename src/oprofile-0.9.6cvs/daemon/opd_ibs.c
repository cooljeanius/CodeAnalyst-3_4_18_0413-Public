...
#ifdef WITH_CA_AGENT
extern unsigned long ca_jit_stats[];
#endif
...
static unsigned long ibs_op_invalid_rip_stats;
...
/* CALOG stuff */
struct calog * dcmiss_log;
struct calog * bta_log;
...
#ifdef WITH_CA_AGENT
	/* check the ca_jit map before treating the samples as anonymous samples */
	if (!trans->in_kernel && trans->cookie == NO_COOKIE) {
		/* Need to read the offset to the .text section */
		trans->ca_jit = find_ca_jit_mapping(trans);
		if (trans->ca_jit) {
			verbprintf(vmisc, 
				"CA JIT Matched (range 0x%lx-0x%lx, tgid %u, pc %lx)\n",
				(unsigned long)trans->ca_jit->start, 
				(unsigned long)trans->ca_jit->end, 
				(unsigned int)trans->tgid, 
				(unsigned long)trans->pc); 

			// Adding Jit header size
			trans->pc += trans->ca_jit->elf_header_size;

			ca_jit_stats[CA_JIT_SAMPLE]++;

			/* For ca_jit, sfile change when ca_jit module changes*/
			if(trans->current == NULL
			|| trans->current->ca_jit == NULL
			|| trans->current->ca_jit->start != trans->ca_jit->start) {
				trans->current = NULL;
				trans->anon    = NULL;
			}
		} else {
			// If failed to map ca_jit, handle as anon sample
			clear_trans_current(trans);
...
out:
	...
	if (trans->event == CALLGRAPH_EVENT) {
		trans->ca_tracing_depth++;
	} else {
		trans->ca_tracing_depth = 0;
	}
#else
...
static int ibs_init(char const * argv)
{
	...
	// EXPERIMENT: DCMiss Info log	
	if (ibs_op_um & 0x2) {
		char filename[1024];
		strncpy(filename, session_dir, 1023);
		strncat(filename, "/samples/ibs_dcmiss.log", 1024);
		dcmiss_log = calog_init(filename, CALOG_DCMISS);
	}
	...
	if (ibs_bta_enabled) {
		...
		bta_log = calog_init(filename, CALOG_BTA);
	}
...
static int ibs_print_stats()
{
	...
	printf("Nr. IBS Op invalid RIP    : %lu\n", ibs_op_invalid_rip_stats);
