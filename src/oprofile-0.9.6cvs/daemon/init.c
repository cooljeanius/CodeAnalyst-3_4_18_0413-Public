...
static int opd_get_mux_norm()
{
	int mux_norm;
	char filename[] = "/dev/oprofile/mux_norm";
	mux_norm = op_read_int_from_file(filename, 0);
	if (mux_norm <= 0)
		mux_norm = 1;
	return mux_norm;
}
...
static void opd_26_init(void)
{
	...
	opd_mux_norm = opd_get_mux_norm();
	printf("Multiplexing normalization value: %d\n", opd_mux_norm);
}
