...
enum tracing_type {
	...
	TRACING_ON,
#ifdef ENABLE_CA_CSS
	CALLGRAPH_EVENT = (unsigned long)-1,
#endif
};
...
struct transient {
	...
	void * ext;
#ifdef WITH_CA_AGENT
	struct ca_jit_mapping * ca_jit;
	struct ca_jit_mapping * last_ca_jit;
#endif
#ifdef ENABLE_CA_CSS
	unsigned int ca_tracing_depth;
	cookie_t ca_last_cookie;
#endif
};
...
#ifdef WITH_CA_AGENT
	trans->last_ca_jit = trans->last_ca_jit;
#endif
#ifdef ENABLE_CA_CSS
	trans->ca_last_cookie = trans->cookie;
#endif
}
