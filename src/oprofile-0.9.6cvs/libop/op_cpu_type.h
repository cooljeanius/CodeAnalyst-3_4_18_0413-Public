...
typedef enum {
	...
	CPU_FAMILY15H_1XH, /**< AMD family 15h model 10-1Fh */
	MAX_CPU_TYPE
} op_cpu;
...
/**
 * get the number of multiplexing 
 * @param cpu_type numeric processor type
 *
 * returns 0 if the CPU does not support mux 
 */
int op_get_nr_mux(op_cpu cpu_type);
 ...
 *
 */
int op_get_counter_mask(unsigned int * mask);
